...
//$PAMTC,EN,S // save to EEPROM (changing wind sensor settings only affects RAM; unless this command is used, settings will return to previous state when power is cycled)
  ...
  sensorData(BUFF_MAX, 'w');
  Serial.println("Booting up, waiting 2 seconds then getting current GPS point");
  Serial.println("will keep spitting out gps coordinates every 2 seconds until you press any key");
  
  do{
  delay(2000);
  sensorData(BUFF_MAX, 'w');
  Serial.print("current gps coordinate is:  ");
  ...
  startingPoint.latDeg = boatLocation.latDeg;
  startingPoint.latMin = boatLocation.latMin;
  startingPoint.lonDeg = boatLocation.lonDeg;
  startingPoint.lonMin = boatLocation.lonMin;
}
...
void loop()
{
  ...
  {
    count++;
    distance = GPSdistance(startingPoint, boatLocation);
    direction2 = getWaypointDirn(startingPoint);
